...
print(folder_to_track)# in the setting file
print(countdown_time)# in the setting file
print("#########################################")
print(time_out)# in the setting file
print(script_fm_path)# in the setting file
...
def NAS_fm_log(text): # Save NAS_fm_log.txt File
    ...
    try:
        with open(log_file_path,"a") as file:
            ...
            print("Time:" + get_current_time() + "\t" + text + "\n")
    except:
        print("error in writing NAS_fm_log.txt")
NAS_fm_log("DCECFERFVREV")
NAS_fm_log("QSWDEFCRDVFD")
NAS_fm_log("CASFDFDVFGFF")

def load_data(path, file_name):# this function only be used in row 212
    global data0
    data0 = utils.CalibrationData([path], file_name, uniquify=False, typify=False, verbose=False)
    print(path)
    print(file_name)
    print(data0)
# load_data(script_fm_path+'/BATCH_B12L0001_test_1502/', Test_summary_30deg.tsv)
# ？load_data的path和file_name是将被上传的数据的path和文件名吗？
        ...
        for path in pointers:
            ...
            if data['Pending to Parse'] == True:
                ...
                if file_key != 'module_log':
                    try:
                        load_data(data['Path'], FILENAME)# use the function defined in row 138
                                                         # only use here
                    except: #针对以前存在过，但是后来又被删除的情况
                        with open(script_fm_path + "pointers.json","r") as file:
                            pointers = json.load(file)
                            data = pointers[path]
                ...
                else: # 另一个分支:if file_key != 'module_log':
                    df = pd.read_excel(path)
                    try: # if e-test file has at least one data line
                        df_sfc = get_sfc_frame_module_log(df)#函数在426行
...
def get_sfc_frame_module_log(df, add_columns=add_sfc_columns):
    try:
        ...
        df.loc[df.moduleType=='Cougar with MI0802M5S', 'moduleType'] = 19 #added, about cougar
        df.loc[df.moduleType=='Cougar with MI0802M6S', 'moduleType'] = 20 #added, about cougar
        df.loc[df.moduleType=='Cougar with MI0802M7G', 'moduleType'] = 21 #added, about cougar
